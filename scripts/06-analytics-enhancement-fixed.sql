-- Enhanced Analytics Schema - Simplified Version
-- Run this after your existing setup scripts
-- This version avoids column reference issues
...
DROP POLICY IF EXISTS "Users can view own learning streaks" ON public.learning_streaks;
CREATE POLICY "Users can view own learning streaks" ON public.learning_streaks
  FOR SELECT USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can insert own learning streaks" ON public.learning_streaks;
CREATE POLICY "Users can insert own learning streaks" ON public.learning_streaks
  FOR INSERT WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can update own learning streaks" ON public.learning_streaks;
...
-- Create simplified analytics summary view
...
SELECT 
  ...
  -- Recent activity using correct column names
...
-- Success message
SELECT 'Analytics enhancement completed successfully!' as status;